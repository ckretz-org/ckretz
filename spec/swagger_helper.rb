...
RSpec.configure do |config|
  ...
  def model(type:)
    name = "Commands::#{type.to_s.camelcase}::Create"
      ...
      components: {
        securitySchemes: {
          keycloak: {
            ...
            flows: {
              implicit: {
                authorizationUrl: ENV.fetch("KEYCLOAK_AUTH_SERVER_URL", "http://login.spexops.local:8080") + "/realms/spexops/protocol/openid-connect/auth",
                scopes: {}
              }
            }
          }
